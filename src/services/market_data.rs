...
pub async fn fetch_6month_data_for_chart(quote: &StockQuoteData) -> Option<StockChartData> {
    let raw_data_mock = get_mock_6month_data(quote.open);
...
fn get_mock_6month_data(start_price: Option<f64>) -> HashMap<String, HashMap<String, String>> {
    let mut data = HashMap::new();
    let mut rng = rand::thread_rng();
    let mut current_price = start_price.unwrap_or(100.0);
    let today = Utc::now().date_naive();

    for day_offset in 0..180 {
        let date = today.pred_opt().and_then(|d| d.checked_sub_days(chrono::Days::new(day_offset as u64)));
        let date = if let Some(d) = date {
            d
        } else {
